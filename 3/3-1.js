...
const wire1SetPath = new Set(wire1Path.map(coord => `${coord[0]},${coord[1]}`));
...
const wire2SetPath = new Set(wire2Path.map(coord => `${coord[0]},${coord[1]}`));

wire2SetPath.forEach((wire2position) => {
  if (wire1SetPath.has(wire2position)) {
