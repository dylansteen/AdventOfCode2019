...
wire1.forEach((currentInstruction) => {
  ...
  let length = currentInstruction.slice(1);
...
const shortestWirePath = collisionSteps.reduce((current, next) => {
  if (next < current) {
    return next;
  }
  return current;
